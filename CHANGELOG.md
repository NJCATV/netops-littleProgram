...
- 新增统一工单列表、创建、详情、状态动作和备注接口。
- 新增智能装维施工实例与轮次启动接口。
- 新增统一工单 API 状态机和权限范围测试。

### Fixed

- 补齐服务器资产分组和扩展字段 ORM，消除数据库迁移与模型结构漂移。

## 2026-08-12

### Added

- 新增数据库 V2 用户、组织成员、RBAC、外部账号与身份匹配基础模型。
- 新增统一工单分派、OSS引用与同步、智能装维全流程、统一文件和批量导出模型。
- 新增全新项目幂等初始化测试和智能装维数据库 V2 设计文档。

### Changed

- 项目改为全新数据库建设，不迁移旧库业务数据。
- Alembic 迁移链修复为单一 head，旧 SQLite 不兼容约束迁移改为 batch 模式。
- 初始化默认不再生成示例服务器；OSS 测试身份通过环境变量加密绑定到管理员。
...
- 小程序缓存的旧 `/api`、`/wx/api` 或 `172.31.1.233:5772/7001` 内网基址会自动迁移到正式域名下的 `/api/netops2026`，避免真机保留历史配置后继续请求退役或未加入微信合法域名的接口。
