...
## 2026-08-19 - Miniapp Review Simplification

### Added

- 新增 AIOps 知识库、AIOps 系统管理、系统审计和基础设施监控四个移动端页面。
- 底部导航新增中间“一键 AI”入口；智能装维入口保持正常图标，点击进入“敬请期待”占位页。

### Changed

- 工作台重分为施工、查询、监测、网管系统、现场工具和平台管理，顶部身份区改为紧凑卡片。
- 菜单分组删除说明性副标题；微信底栏改为图形化自定义导航，中间 AI 按钮增加抬升、渐变与呼吸光晕。
- 动态导航覆盖全部授权菜单，每次进入工作台均重新按当前账号取权；底栏激活态改为以当前页面为准，解决需要重复点击的问题。
- 权限目录统一校正名称、路径、分组与排序；“网管配置”限定超级管理员，清理服务器管理等无实际入口，并支持删除非核心配置项。
- 头像上传改为微信标准 `chooseAvatar` 入口、压缩和前后端校验，统一历史/当前图片路径，服务端保存明确的移动端地址并重新编码为去元数据 JPEG。
- 微信开发者工具项目根目录固定指向 `miniapp/dist/build/mp-weixin`，避免误调试旧原生小程序或过期构建包。
- 质差趋势与百行统计表改为适合手机浏览的横向趋势和分批排行卡片。

### Fixed

- 修复 OLT 三级选择器在微信小程序中无法纵向滑动的问题。
- 菜单禁用现在同时限制入口和对应后端功能接口，角色、用户类型与启停开关形成统一权限门禁。
- 修复菜单初始化反复覆盖管理员权限配置及跨账号缓存残留的问题；已知功能缺失配置时改为拒绝访问。
- 修复头像上传成功后仍被旧反向代理路径读取、最终显示为加载失败的问题；上传完成后增加远端图片可读性确认。
- 历史头像文件缺失时仅回退为文字头像，不再弹出容易被误解为上传失败的异常提示。
- 修复头像选择器返回页面触发旧资料刷新、延迟图片错误事件覆盖新头像的问题。
- 登录账号或密码错误、频率限制等提示改为中文。
