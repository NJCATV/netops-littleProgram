...
- 修复头像历史路径拼接兼容与图片加载失败回退；上传前端和后端共同限制 JPG/PNG/WebP、2MB、宽高 128～4096 像素，服务端按图片真实内容校验。
- 将 AIOps 知识库、AIOps 系统管理、系统审计与使用分析、基础设施监控迁移为移动端页面。
- 修复菜单 10 分钟缓存导致禁用后仍显示的问题；导航改为返回全部已授权菜单；后端将功能接口统一绑定到菜单启停、角色和用户类型权限。
- 质差页 OLT 三级选择器补齐固定可滚动高度；趋势改为横向可滑动堆叠柱；两个百行统计表改为移动卡片并默认展示前 10 条、按需继续加载。
- 验证：uni-app 微信小程序生产构建通过；本轮后端 16 项全量测试通过（新增头像上传 3 项）；390×844 H5 视口检查无页面横向溢出，OLT 选择器正常展开。
