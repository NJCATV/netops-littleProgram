...
## 2026-08-19 - 小程序审核前简化、权限闭环与质差页适配

- 工作台按施工服务、业务查询、运行监测、网管系统、现场工具、平台管理重新分类，压缩顶部用户信息，并将 AI 运维助手改为底部中间独立入口。
- 智能装维保留完整功能源码，工作台入口正常展示且无“敬请期待”角标；点击仅进入独立“敬请期待”占位页，审核包不注册未完成功能页。
- 菜单分组删除说明性副标题，卡片进一步压缩；微信底栏改为自定义图形导航，中间 AI 入口使用抬升渐变按钮和呼吸光晕。
- 头像改用微信标准 `chooseAvatar` 选择与裁切入口；上传前压缩和校验，上传后验证远端图片可读取，并将 `/api/files`、`/files` 等历史路径统一到移动端命名空间。
- `miniapp/project.config.json` 明确将开发者工具根目录设为最新构建输出；历史头像读取失败静默回退，智能装维跳转兜底文案固定为“功能正在完善中，敬请期待”。
- 将 AIOps 知识库、AIOps 系统管理、系统审计与使用分析、基础设施监控迁移为移动端页面。
- 修复菜单缓存和初始化脚本覆盖导致的越权显示；“网管配置”固定为超级管理员，菜单名称、路径、分组和排序经迁移统一校正，删除服务器管理等无实际入口并增加非核心项删除能力。
- 工作台不再跨账号复用菜单缓存；已知功能缺失菜单配置时接口默认拒绝，权限配置自身不可删除、禁用或改名绕过保护。
- 底部菜单激活态改由当前页面生命周期同步，增加按压反馈；登录错误、频率限制和改密错误提示完成中文映射。
- 质差页 OLT 三级选择器补齐固定可滚动高度；趋势改为横向可滑动堆叠柱；两个百行统计表改为移动卡片并默认展示前 10 条、按需继续加载。
- 验证：uni-app 微信小程序生产构建通过；后端 19 项全量测试、Python 编译、空库及模拟存量库 Alembic 升级通过；390×844 H5 视口检查无页面横向溢出，OLT 选择器正常展开。
- 已将提交 `eee0d35` 的相关后端文件定向部署到 JSCN-233，保留线上 Radius/AIOps 定制；生产 MySQL 迁移至 `d8e9f0a1b2c3`，菜单由 36 项清理为 32 项，“网管配置”限定超级管理员，“智能装维”指向审核占位页。
- 生产环境安装 Pillow 10.3.0 并完成头像重编码链路验证；Gunicorn 无中断重载后内部健康检查 200、公网鉴权链路 401 符合预期，代码与菜单备份位于 `backups/deploy-20260819-eee0d35`。
- 修复微信头像选择器返回页面时的异步竞态：上传开始后使旧的用户信息请求失效，并复核延迟到达的图片错误事件，避免新头像短暂显示后又被旧资料或文字头像覆盖；微信小程序生产构建通过。

## 2026-08-12 - 智能装维融合现有智维平台纠偏

- 确认生产唯一数据库为 `anbo_wx`，复用 1646 个现有用户、225 个组织及 `users.role_code`、`users.oss_*` 字段。
- 移除误建的平行角色、用户角色、组织成员、外部账号和身份匹配模型及迁移。
- Alembic 改为从现有生产版本原位新增统一工单和智能装维表，OA 字段迁移兼容生产库已有字段。
- 初始化脚本只补齐根组织和菜单，明确禁止创建、修改管理员或 OSS 测试账号。
- 智维平台 uni-app 工作台新增智能装维入口，完成工单领取、详情、五项检测、签字和 OSS 回单页面。
- 验证：后端 18 项测试通过；微信小程序生产构建通过。

## 2026-08-12 - 智能装维证据与 AIOps 执行闭环

- 新增五类施工照片受控上传与鉴权下载，文件头、大小、数量、角色、坐标和水印结构均在服务端校验。
- 新增业务后端到 AIOps 的最小权限签名调用，浏览器和小程序不接触共享密钥，也不直连 AIOps。
- 智能体运行先落 `pending`，成功后固化版本号、配置快照、事实和逐项评分；失败保留脱敏错误。
- 工单详情补充各施工轮次的照片和 AI 运行摘要；重新施工仍保留历史轮次证据。
- 新增 `installation.agent.run` 权限，普通施工人员仅运行，组织管理员以上可管理配置。
- 验证：后端 13 项测试通过，覆盖签名最小权限、照片上传/鉴权读取和 AI 结果持久化；数据库无迁移漂移。

## 2026-08-12 - OSS 接单、回单与可靠重试

- 新增 OSS 已领取工单查询和领取接口，领取成功后幂等同步到统一工单池。
- 新增完工回单队列；回单前校验工单可见范围、完工状态和当前施工轮次客户签字。
- 新增 OSS outbox 后台派发脚本、分级退避、失败封顶和管理员单条重试接口。
- 接单和回单均使用业务幂等键并写 `oss_sync_logs`，凭据和 token 不进入事件载荷或日志。
- 验证：后端 10 项测试通过，数据库无迁移漂移；真实 OSS 仍只执行只读回归，写操作由模拟测试覆盖。

## 2026-08-12 - OSS只读查询与统一入池适配

- 新增基于 `external_accounts` 的 OSS 登录和业务请求适配层，凭据解密与 token 均只在服务端执行。
- 新增待办工单只读查询、工单详情和原始工单同步入统一工单池接口。
- OSS 待办查询自动使用登录资料补齐施工区域、本地网、区域、运行状态和分页参数。
- 使用管理员测试绑定做正式 OSS 只读回归：返回 `returnCode=0`，首批 5 条，未执行领取或回单。
- OSS 工单重复同步测试保持一张统一工单和一条外部引用；后端测试共 7 项通过。
