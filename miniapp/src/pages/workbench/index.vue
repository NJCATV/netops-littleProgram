  ...
  <view class="page workbench-page">
    ...
    <view v-else class="groups">
      <view v-for="group in visibleGroups" :key="group.group_name" class="group-card">
        <view class="group-head"><view><view class="group-name">{{ group.group_name }}</view><view v-if="group.description" class="group-description">{{ group.description }}</view></view><view class="group-count">{{ group.items.length }} 项</view></view>
        <view class="grid">
          <view v-for="item in group.items" :key="item.menu_key" class="entry" :class="{ pending: item.comingSoon }" @tap="go(item)">
            <view class="entry-icon" :class="`icon-${iconFor(item)}`">{{ iconText(iconFor(item)) }}</view><view class="entry-name">{{ item.name }}</view><view v-if="item.comingSoon" class="coming-badge">敬请期待</view>
          </view>
        </view>
      </view>
    </view>
...
const user = ref(getStoredUser()); const groups = ref([]); const keyword = ref(''); const loading = ref(false)
const initial = computed(() => (user.value.real_name || '用').slice(0, 1)); const avatar = computed(() => resolveAssetUrl(user.value.avatar_url))
...
function organizeGroups(items) {
  ...
  const result = []
  if (buckets.construction.length) result.push({ group_name: '施工服务', description: '现场施工能力', items: buckets.construction })
  if (buckets.query.length) result.push({ group_name: '业务查询', description: '快速定位用户与接入信息', items: buckets.query })
  if (buckets.monitor.length) result.push({ group_name: '运行监测', description: '状态、质量与性能监测', items: buckets.monitor })
  if (buckets.netSystem.length) result.push({ group_name: '网管系统', description: '设备、AIOps 与平台治理', items: buckets.netSystem })
  if (buckets.field.length) result.push({ group_name: '现场工具', description: '值班与随身工具', items: buckets.field })
  if (buckets.platform.length) result.push({ group_name: '平台管理', description: '账号、组织与权限配置', items: buckets.platform })
  if (buckets.other.length) result.push({ group_name: '其他功能', items: buckets.other })
  return result
}
function go(item) {
  if (item.comingSoon) { uni.showToast({ title: '智能装维正在完善，敬请期待', icon: 'none' }); return }
